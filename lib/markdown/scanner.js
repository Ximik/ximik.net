...
export default class Scanner {
  ...
  scanToken() {
    ...
    if ( bol && char === EXCLAMATION && this.skipString(EXCLAMATION + EXCLAMATION)) {
      const packages = this.readTillString(NEWLINE).split(" ").filter((p) => p);
      const text = this.readTillString( NEWLINE + EXCLAMATION + EXCLAMATION + EXCLAMATION);
      return { type: TIKZ, packages, text };
    }
    if (char === EXCLAMATION) {
      const text = this.readTillString(RBRACKET, false);
      if (!this.skipString(LPARENTHESIS)) {
        return { type: TEXT, text: EXCLAMATION + text + RBRACKET };
      }
      const alt = text.slice(1, text.length) //?!
      const src = this.readTillString(RPARENTHESIS);
